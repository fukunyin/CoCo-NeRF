...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary, level=0):
    ...
    surface_traces = get_surface_trace(path=path,
                                       epoch=epoch,
                                       sdf=lambda x: implicit_network(x)[0, 0, :, 0],
...
def get_surface_trace(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0):
    ...
    for i, pnts in enumerate(torch.split(points, 100000, dim=0)):
        outputs = sdf(pnts[None, None])
        z.append(outputs.cpu().numpy())
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res):
    ...
    ground_true = lin2img2(ground_true, img_res).cpu().squeeze().numpy()
    ground_true = (ground_true * scale_factor).astype(np.uint8)
    img = Image.fromarray(ground_true)
    img.save('{0}/rendering_gt{1}.png'.format(path, epoch))

    ground_true = lin2img2(rgb_points, img_res).cpu().squeeze().numpy()
    ground_true = (ground_true * scale_factor).astype(np.uint8)
    img = Image.fromarray(ground_true)
    img.save('{0}/rendering_img{1}.png'.format(path, epoch))
...
def lin2img2(tensor, img_res):
    batch_size, num_samples, channels = tensor.shape
    return tensor.reshape(img_res[0], img_res[1], channels)
